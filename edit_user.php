...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username)) $errors[] = "Kullanıcı adı zorunludur.";
    ...
    if (empty($full_name)) $errors[] = "Ad Soyad zorunludur.";
    if (!in_array($role, ['admin', 'user'])) $errors[] = "Geçerli bir kullanıcı rolü seçiniz.";
    if (!empty($password)) {
        if (strlen($password) < 6) $errors[] = "Şifre en az 6 karakter olmalıdır.";
        elseif ($password !== $password_confirm) $errors[] = "Şifreler eşleşmiyor.";
    }
    // --- Benzersizlik kontrolleri (önceki kod aynı) ---
     if (empty($errors)) {
        try {
            $stmtCheck = $conn->prepare("SELECT COUNT(*) FROM users WHERE username = :username AND id != :id");
            $stmtCheck->bindParam(':username', $username); $stmtCheck->bindParam(':id', $user_id); $stmtCheck->execute();
            if ($stmtCheck->fetchColumn() > 0) $errors[] = "Bu kullanıcı adı zaten kullanılıyor.";

            $stmtCheck = $conn->prepare("SELECT COUNT(*) FROM users WHERE email = :email AND id != :id");
            $stmtCheck->bindParam(':email', $email); $stmtCheck->bindParam(':id', $user_id); $stmtCheck->execute();
            if ($stmtCheck->fetchColumn() > 0) $errors[] = "Bu e-posta adresi zaten kullanılıyor.";
        } catch(PDOException $e) { $errors[] = "Veritabanı hatası (benzersizlik kontrolü): " . $e->getMessage(); }
    }
                 ...
                 <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . '?id=' . $user_id); ?>" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="role" name="role" required <?php echo ($user['id'] == $_SESSION['user_id'] && $user['role'] == 'admin') ? 'disabled' : ''; ?>>
                             ...
                             <?php if ($user['id'] == $_SESSION['user_id'] && $user['role'] == 'admin'): ?>
                                ...
                                <input type="hidden" name="role" value="admin"> <!-- Form gönderildiğinde rolün değişmemesi için -->
                            <?php endif; ?>
                        </div>
                    </div>
                     <hr>
                    ...
                    <h5 class="mb-3">E-posta İmzası Ayarları</h5>
                     <div class="mb-3">
                        ...
                        <textarea class="form-control" id="email_signature" name="email_signature" rows="4"><?php echo htmlspecialchars($user['email_signature'] ?? ''); ?></textarea>
                        <div class="form-text">E-postaların sonuna eklenecek metin. Stil (yazı tipi, renk) `mailto:` ile kullanılamaz, sadece sunucu taraflı gönderimde uygulanır.</div>
                    </div>
                     <div class="mb-3">
                         <label for="signature_image" class="form-label">İmza Resmi</label>
                         <input class="form-control" type="file" id="signature_image" name="signature_image" accept="image/png, image/jpeg, image/gif">
                         <div class="form-text">Mevcut resmi değiştirmek için yeni bir resim seçin (PNG, JPG, GIF - Maks 2MB).</div>
                     </div>
                     ...
                     <?php if (!empty($user['signature_image'])): ?>
                        <div class="mb-3">
                             ...
                             <div>
                                 <img src="<?php echo SIGNATURE_UPLOAD_DIR . htmlspecialchars($user['signature_image']); ?>?t=<?php echo time(); // Cache'i önlemek için ?>" alt="İmza Resmi" class="signature-preview">
                             </div>
                             <div class="form-check mt-2">
                                 <input class="form-check-input" type="checkbox" value="1" id="delete_signature_image" name="delete_signature_image">
